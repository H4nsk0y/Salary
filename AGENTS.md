...
- задачи отдела: `tasks.html` + `tasks.js` (временно заморожены и закрыты)
- инструкции ЕГАИС: `instructions.html` + `instructions.js` (временно заморожены и закрыты)
...
Анонимное голосование за сотрудника недели и месяца находится в `voting.html` + `voting.js`, но временно заморожено и заменено в навигации личным чек-листом смены. Код и таблицы голосования не удалять; старый адрес переводит пользователя на `checklist.html`. Идентификатор голосующего никогда не должен возвращаться клиенту, а прямой доступ `authenticated` к таблицам голосования должен оставаться закрытым: работа идет только через RPC из `031_staff_voting.sql`.
...
Задачи отдела и инструкции ЕГАИС также считаются замороженными функциями: исходники и таблицы не удалять, но ссылки не показывать, а прямой переход на `tasks.html` или `instructions.html` должен возвращать на главную. Не развивать эти разделы без явной просьбы пользователя.
...
`staff_votes` / `staff_vote_results`
- анонимные недельные и месячные голосования за сотрудников
- `staff_votes.voter_user_id` хранится только для ограничения одного голоса и не возвращается клиенту
- активные итоги не показываются; завершенный результат фиксируется один раз, включая случайный выбор победителя при ничьей
- комментарии становятся доступны только после завершения периода
- схема и RPC находятся в `031_staff_voting.sql`
