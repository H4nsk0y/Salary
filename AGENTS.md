...
Основное меню сотрудника: калькулятор, личный табель, смены, голосование, профиль. Страница новостей доступна из профиля и не дублируется в общем header.

Анонимное голосование за сотрудника недели и месяца находится в `voting.html` + `voting.js`. Один аккаунт может отдать один голос за каждый период, голосовать за себя нельзя. Текущие результаты скрыты до окончания периода. Идентификатор голосующего никогда не должен возвращаться клиенту, а прямой доступ `authenticated` к таблицам голосования должен оставаться закрытым: работа идет только через RPC из `031_staff_voting.sql`.
