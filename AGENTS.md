...
Интерактивное обучение заполнению табеля находится в `timesheet-training.html` + `timesheet-training.js`. Оно не сохраняет данные в Supabase: профиль читается только для выбора режима нормы, а прогресс хранится локально под ключом `alvisa.timesheetTraining.v1`. Старый Driver.js-тур остаётся обзором элементов реальных страниц, а не практикой заполнения.
